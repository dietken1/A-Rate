...
public interface SharedMaterialRepository extends JpaRepository<SharedMaterial, Long> {
    ...
    Page<SharedMaterial> findByLecture_Id(Long lectureId, Pageable pageable);
    
    List<SharedMaterial> findByUploader_Id(Long uploaderId);
    
    // 최신 공유자료 조회 (대시보드용)
    List<SharedMaterial> findTopByOrderByCreatedAtDesc();
    ...
    Page<SharedMaterial> findByTagsContaining(@Param("tag") String tag, Pageable pageable);
